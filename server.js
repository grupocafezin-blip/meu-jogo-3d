...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({roomId, name}, cb) => {
    ...
    rooms[roomId] = { players: {}, multiplier: 1, level: 1, status: 'lobby', startLives: 6 };
    socket.join(roomId);
    rooms[roomId].players[socket.id] = { id: socket.id, name: name||'Player', score:0, ready:false };
  ...
  socket.on('joinRoom', ({roomId, name}, cb) => {
    ...
    socket.join(roomId);
    room.players[socket.id] = { id: socket.id, name: name||'Player', score:0, ready:false };
    socket.data.roomId = roomId;
  ...
  // Toggle player's ready state and start the match if everyone is ready
  socket.on('setReady', (ready, cb) => {
    ...
    p.ready = !!ready;
    io.to(roomId).emit('roomUpdate', room);
    // If all players in room are ready, start game once
    const players = Object.values(room.players);
    if (players.length > 0 && players.every(pl => pl.ready) && room.status !== 'playing') {
      room.status = 'playing';
      room.level = 1;
      room.multiplier = 1;
      // reset scores
      players.forEach(pl => { pl.score = 0; });
      io.to(roomId).emit('startGame', { startLives: room.startLives, level: room.level });
      io.to(roomId).emit('roomUpdate', room);
    }
    cb && cb({ok:true, room});
  });
